...
class MemberReportsTable extends Component {
  ...
  download() {
    ...
    userService.downloadfile("exportReportMember", this.state.url).then(result => {
      console.log(result);
      if (!result.success) {
        cogoToast.warn(
            ...
            <MaterialTable
...

                {
                  ...
                  render: (rowData) => (
                    <div className="table_personel">
                      ...
                      <small>{dayjs(rowData.log_time).locale('tr').format('DD/MM/YYYY HH:mm')}</small>
                    </div>
              ...
              data={(query) =>
                new Promise((resolve, reject) => {
                  ...
                  userService.getMemberReportsStatus(url).then(result => {
                  ...
                  userService.getMemberReports(url).then(result => {
                    if (result.error) {
                      cogoToast.warn(
                        "Webservis Kaynaklı Sorun Oluşmuştur, Lütfen Sistem Yöneticisine Bilgi Veriniz...",
                        { position: "bottom-right" }
...
export default withRouter(MemberReportsTable);
